...
if isfield(ps,'exc')
    ...
    % column names
    fprintf(fid,'%%');
    for i=1:length(C.ex.col_names)
        fprintf(fid,'%s ',C.ex.col_names{i});
    end
    fprintf(fid,'\n');
    % data
    for i = 1:size(ps.exc,1)
        fprintf( fid,' %g',ps.exc(i,:) );
        fprintf( fid,';\n' );
    end
    fprintf(fid,'];\n\n');
end
...
if isfield(ps,'gov')
    ...
    % column names
    fprintf(fid,'%%');
    for i=1:length(C.go.col_names)
        fprintf(fid,'%s ',C.go.col_names{i});
    end
    fprintf(fid,'\n');
    % data
    for i = 1:size(ps.gov,1)
        fprintf( fid,' %g',ps.gov(i,:) );
        fprintf( fid,';\n' );
    end
    fprintf(fid,'];\n\n');
end
