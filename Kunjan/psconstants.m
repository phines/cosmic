...
if isempty(C)
	...
	C.bu.cols = 20; % minimum number of columns
    ...
    C.sh.id     = 11; % unique id number for this shunt element.
	C.sh.cols   = 11; % minimum number of columns
    % synonyms:
    
	C.shunt     = C.sh; % synonym
    C.sh.col_names = {'bus','P','Q','frac_S','frac_Z','status','type','value','frac_E','gamma','id'};
    ...
    C.event.cols = 8;
    C.event.col_names = {'time','type','bus_loc','branch_loc','gen_loc','shunt_loc','relay_loc','quantity'};
    % synonym
    C.ev = C.event;
    ...
    C.relay.id              = 13;   % unique id number for the relay
    C.relay.cols = 13;
    % syn
    C.re = C.relay;
end
