...
if ~all(is_first_subgraph)
    ...
    ps1  = subsetps(ps,net1);   [x01,y01] = get_xy(ps1);
    ps2  = subsetps(ps,net2);   [x02,y02] = get_xy(ps2);
...
elseif (n_macs == 0 || n_shunts == 0 || (ps.shunt(:,C.sh.P)'*ps.shunt(:,C.sh.factor) == 0) || size(ps.bus(:,1),1) == 1)
    ...
    ix          = get_indices(n,n_macs,m,n_shunts);
    ...
    if isempty(y_new)
        ...
        switch opt.sim.integration_scheme
            case 1
                % trapezoidal rule
                clear fn_f; clear fn_g;
                fn_f = @(t,x,y) differential_eqs(t,x,y,ps,opt);
                fn_g = @(t,x,y) algebraic_eqs(t,x,y,ps);
                [t_ode,X,Y,Z] = solve_dae(fn_f,fn_g,[],x0,y0,t:opt.sim.dt_default:t_next,opt); 
                %if ~isempty(Z) && size(X,2)>tail
                    % trash the voltage collapse tail
                %    t_ode(end-tail:end) = [];
                %    X(:,end-tail:end)   = [];
                %    Y(:,end-tail:end)   = []; 
                %end
        ...
        ps.bus(1,C.bus.delta_sys)   = delta_sys;
        ...
        if ~isempty(relay_event)
            ...
            while ~isempty(relay_event)
                % handle voltage collapse separately (for now)
                bus_id = ps.bus_i(relay_event);
                shunt_bus_ix = ismember(ps.shunt(:,1),bus_id);
                prev_load = ps.shunt(:,C.sh.P).*ps.shunt(:,C.sh.factor);
                ps.shunt(shunt_bus_ix,C.sh.factor) = max(ps.shunt(shunt_bus_ix,C.sh.factor) - 0.25, 0);
                curr_load = ps.shunt(:,C.sh.P).*ps.shunt(:,C.sh.factor);
                fprintf('  t = %.2f: %.2f MW of load shedding...\n',t_event,sum(prev_load)-sum(curr_load));
                relay_event = [];
                %new_event = zeros(1,C.ev.cols);
                %new_event(C.ev.time) = t_event;
                %new_event(C.ev.type) = C.ev.relay_trigger;
                %new_event(C.ev.relay_loc) = relay_event(1);
