function [g,dg_dx,dg_dy] = algebraic_eqs(t,x,y,ps,~) %#ok<INUSL>
...
if nargout>1
    ...
    dg_dx = dg_dx + sparse(Pg_rows,ix.x.delta,-dPg_ddelta_values,ix.ny,ix.nx);
    % dPg_dEa
    dg_dx = dg_dx + sparse(Pg_rows,ix.x.Eap,-dPg_dEa_values,ix.ny,ix.nx);
    % dQg_ddelta
    Qg_rows = ix.g.Q(mac_bus_i);
    dg_dx = dg_dx + sparse(Qg_rows,ix.x.delta,-dQg_ddelta_values,ix.ny,ix.nx);
    % dQg_dEa
...
if nargout>2
    ...
    dg_dy = dg_dy + sparse(ix.g.P(mac_bus_i),ix.y.theta(mac_bus_i),+dPg_ddelta_values,ix.ny,ix.ny);
    dg_dy = dg_dy + sparse(ix.g.Q(mac_bus_i),ix.y.theta(mac_bus_i),+dQg_ddelta_values,ix.ny,ix.ny);
