function [ps,t_out,X,Y] = superset_odeout_rec(ps,ps1,ps2,t_out1,t_out2,X1,Y1,X2,Y2,opt)
% usage: [ps,t_out,X,Y] = superset_odeout_rec(ps,ps1,ps2,t_out1,t_out2,X1,Y1,X2,Y2,opt)
% for rectangular formulation
...
ps.bus(buses1,C.bu.Vr:C.bu.Vi)  = ps1.bus(:,C.bu.Vr:C.bu.Vi);  % Vr Vi
ps.bus(buses2,C.bu.Vr:C.bu.Vi)  = ps2.bus(:,C.bu.Vr:C.bu.Vi);  % Vr Vi
