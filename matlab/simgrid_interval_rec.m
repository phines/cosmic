function [ps,t_out,X,Y] = simgrid_interval_rec(ps,t,t_next,x0,y0,opt)
% usage: [ps,t_out,X,Y] = simgrid_interval_rec(ps,t,t_next,x0,y0,opt)
% integrate PS DAEs from t to t_next for rectangular formulation, recursively if endogenous events
%
% inputs:
    ...
    if isempty(y_new)
        ...
        switch opt.sim.integration_scheme
            ...
            case 3
                % explicit, ode15s
                clear fn_fg;
                event_handle = @(t,xy) endo_event_rec(t,xy,ix,ps);
                fn_fg = @(t,xy) differential_algebraic_rec(t,xy,ix.nx,ps,opt);
                mass_matrix = sparse(1:ix.nx,1:ix.nx,1,ix.nx+ix.ny,ix.nx+ix.ny);
                options = odeset(   'Mass',mass_matrix, ...
                    'MassSingular','yes', ...
                    'Jacobian', @(t,xy) get_jacobian_rec(t,xy,ix.nx,ps,opt), ...
                    'MaxOrder', 2, ...
                    'BDF', 'off', ...
                    'Stats','off', ...
                    'RelTol', 1e-3, ...
