function [ps,converged,islands] = newpf_rec(ps,opt)
...
Vmag0  = abs(V0);  % maybe we dont need this transition term V0 -Jiajia
theta0 = angle(V0);
Vmag = Vmag0;
theta = theta0;
...
for grid_no = 1:nSubGrids
    ...
    [Vmag,theta,Vr,Vi,success] = subgridPowerFlow_rec(bus_subset,Ybus,Vmag,theta,Vr,Vi,Sg,Sd,pq,pv,ref,part_fact,nr_opts);

% work on this later ---Jiajia
...
Vr(abs(Vr)<eps) = 0;
Vi(abs(Vi)<eps) = 0;
V = Vr + j*Vi; % calculated V in complex form
Vmag = abs(V);
Vmag(Vmag<eps) = 0;
theta = (180/pi)*angle(V);

% bus voltage results
ps.bus(:,C.bu.Vmag) = Vmag;
ps.bus(:,C.bu.Vang) = theta;
